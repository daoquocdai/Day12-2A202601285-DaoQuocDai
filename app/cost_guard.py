...
class CostGuard:
    ...
    def spent(self, user_id: str, month: str | None = None) -> float:
        ...
        (CP3): đọc ``self.client.get(self._key(user_id, month))``.
        Key chưa tồn tại → Redis trả None → hàm này phải trả ``0.0``.
        Nhớ ép kiểu ``float(...)`` vì Redis trả về chuỗi.
        """
        key = self._key(user_id, month)
        spent_str = self.client.get(key)
        return float(spent_str) if spent_str is not None else 0.0
    ...
    def check(
        ...
        (CP3): nếu ``spent(user_id) + estimated_cost > self.budget``
